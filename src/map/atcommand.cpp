...
// â”€â”€ Voice admin commands â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
ACMD_FUNC(voicegrant) {
	...
	if (!voice_bridge_send_grant_license(pl_sd->status.account_id, duration_sec)) {
		clif_displaymessage(fd, "Voice server unavailable â€” license NOT granted.");
		return -1;
	}
	char output[128];
	if (duration_sec > 0)
...
ACMD_FUNC(voicerevoke) {
	...
	if (!pl_sd) {
		...
		clif_displaymessage(fd, "Voice server unavailable â€” revoke NOT sent.");
		return -1;
	}
	char output[128];
...
// @voiceblockers <player> â€” how many distinct players have voice-blocked
...
// @voiceblocks <player> â€” list who the target has voice-blocked (online only,
...
// ¿Ñ§¡ìªÑ¹µÑÇªèÇÂ: Êè§¢éÍ¤ÇÒÁä»ÂÑ§Ë¹éÒµèÒ§ NPC UI
static void atcmd_send_ui(map_session_data *sd, const char *msg) {
	clif_scriptmes(*sd, sd->id, msg);
}

/*=========================================
 * ¤ÓÊÑè§µÃÇ¨ÊÍºÊàµµÑÊ¡ÒÃ»éÍ§¡Ñ¹·Ñé§ËÁ´ (All-in-One Defense Status UI)
 *=========================================*/
ACMD_FUNC(mystatus) {
	char output[CHAT_SIZE_MAX];
	...
	// àÃÔèÁÊÃéÒ§Ë¹éÒµèÒ§ UI (àÊÁ×Í¹à»Ô´ NPC)
	clif_scriptmes(*sd, sd->id, "[ ^0000FF ÃÐººµÃÇ¨ÊÍºÊàµµÑÊàªÔ§ÅÖ¡ ^000000 ]");
	atcmd_send_ui(sd, "ÃÐºº¨ÐáÊ´§¼Å ^FF0000à©¾ÒÐ¡ÒÃµéÒ¹·Ò¹ (Reduction)^000000 ·Õèà¡Ô´¨Ò¡¢Í§ÊÇÁãÊè, ¡ÒÃì´ áÅÐºÑ¾à·èÒ¹Ñé¹");
	atcmd_send_ui(sd, "---------------------------------");

	// --- 1. ËÁÇ´ËÁÙè¡ÒÃµéÒ¹·Ò¹¡ÒÃâ¨ÁµÕËÅÑ¡ (Combat & Magic) ---
	atcmd_send_ui(sd, "^FF8C00[ ¡ÒÃµéÒ¹·Ò¹¾×é¹°Ò¹ ]^000000");
	...
	if (short_resist > battle_config.limit_short_long_def) {
		sprintf(output, "µéÒ¹·Ò¹ÃÐÂÐã¡Åé: ^FF0000%d%% (Max)^000000 ^808080[à¡Ô¹ÁÒ %d%%]^000000", battle_config.limit_short_long_def, short_resist - battle_config.limit_short_long_def);
	} else {
		sprintf(output, "µéÒ¹·Ò¹ÃÐÂÐã¡Åé: ^008000%d%%^000000", short_resist);
	}
	atcmd_send_ui(sd, output);

	if (long_resist > battle_config.limit_short_long_def) {
		sprintf(output, "µéÒ¹·Ò¹ÃÐÂÐä¡Å: ^FF0000%d%% (Max)^000000 ^808080[à¡Ô¹ÁÒ %d%%]^000000", battle_config.limit_short_long_def, long_resist - battle_config.limit_short_long_def);
	} else {
		sprintf(output, "µéÒ¹·Ò¹ÃÐÂÐä¡Å: ^008000%d%%^000000", long_resist);
	}
	atcmd_send_ui(sd, output);

	if (magic_resist > battle_config.limit_magic_def) {
		sprintf(output, "µéÒ¹·Ò¹àÇ·Á¹µÃì: ^FF0000%d%% (Max)^000000 ^808080[à¡Ô¹ÁÒ %d%%]^000000", battle_config.limit_magic_def, magic_resist - battle_config.limit_magic_def);
	} else {
		sprintf(output, "µéÒ¹·Ò¹àÇ·Á¹µÃì: ^008000%d%%^000000", magic_resist);
	}
	atcmd_send_ui(sd, output);
	atcmd_send_ui(sd, "---------------------------------");

	// --- 2. ËÁÇ´ËÁÙè¡ÒÃµéÒ¹·Ò¹à¼èÒ (Race) - à¾ÔèÁ RC_PLAYER_HUMAN àÃÕÂºÃéÍÂ ---
	atcmd_send_ui(sd, "^FF8C00[ ¡ÒÃµéÒ¹·Ò¹à¼èÒ (Race) ]^000000");
	
	// à¾ÔèÁ RC_PLAYER_HUMAN à¢éÒä»ã¹ÅÔÊµìÃÒÂ¡ÒÃàªç¤
	const int check_races[] = { RC_DEMIHUMAN, RC_BRUTE, RC_DEMON, RC_FORMLESS, RC_PLAYER_HUMAN }; 
	const char* race_names[] = { "¡Öè§Á¹ØÉÂì", "ÊÑµÇì", "»ÕÈÒ¨", "äÃéÃÙ»·Ã§", "¼ÙéàÅè¹Á¹ØÉÂì (PvP)" };
	int race_count = sizeof(check_races) / sizeof(check_races[0]); // ¤Ó¹Ç³¨Ó¹Ç¹à¼èÒÍÑµâ¹ÁÑµÔÍÑµâ¹ÁÑµÔ
	...
	for (int i = 0; i < race_count; i++) {
		...
		if (total_race > 0) { 
			if (total_race > battle_config.limit_race_def) {
				sprintf(output, "à¼èÒ %s: ^FF0000%d%% (Max)^000000 ^808080[à¡Ô¹ÁÒ %d%%]^000000", race_names[i], battle_config.limit_race_def, total_race - battle_config.limit_race_def);
			} else {
				sprintf(output, "à¼èÒ %s: ^008000%d%%^000000", race_names[i], total_race);
			}
			atcmd_send_ui(sd, output);
		}
	}
	...
	// --- 3. ËÁÇ´ËÁÙè¡ÒÃµéÒ¹·Ò¹¸ÒµØ (Element) ---
	atcmd_send_ui(sd, "^FF8C00[ ¡ÒÃµéÒ¹·Ò¹¸ÒµØ (Element) ]^000000");
	const char* ele_names[] = { "äÃé¸ÒµØ", "¹éÓ", "´Ô¹", "ä¿", "ÅÁ", "¾ÔÉ", "ÈÑ¡´ÔìÊÔ·¸Ôì", "Á×´", "ÇÔ­­Ò³", "¼Õ´Ôº" };
	...
	for (int i = 0; i < 10; i++) {
		...
		if (total_ele > 0) { 
			if (total_ele > battle_config.limit_element_def) {
				sprintf(output, "¸ÒµØ %s: ^FF0000%d%% (Max)^000000 ^808080[à¡Ô¹ÁÒ %d%%]^000000", ele_names[i], battle_config.limit_element_def, total_ele - battle_config.limit_element_def);
			} else {
				sprintf(output, "¸ÒµØ %s: ^008000%d%%^000000", ele_names[i], total_ele);
			}
			atcmd_send_ui(sd, output);
		}
	}

	// Êè§»ØèÁ "»Ô´" à¾×èÍ¨ºÊ¤ÃÔ»µì UI
...
ACMD_FUNC(refund_collection) {
	...
	// á¡éºÑ¤ Error áÃ¡: ÃÍ§ÃÑº·Ñé§¡ÒÃ¾ÔÁ¾ìà»ç¹ Item ID (µÑÇàÅ¢) áÅÐ AegisName (µÑÇË¹Ñ§Ê×Í)
	...
	int i;
	// µÃÇ¨ÊÍºÇèÒã¹¤ÅÑ§ Collection ÁÕäÍà·ÁªÔé¹¹ÕéÍÂÙè áÅÐÁÕ¨Ó¹Ç¹¾ÍãËé´Ö§¤×¹ËÃ×ÍäÁè
	...
	// á¡éºÑ¤ Error ÊÍ§: à»ÅÕèÂ¹¨Ò¡ target_sd->bl.m à»ç¹ target_sd->m
