...
void rune_save(map_session_data* sd) {
	...
	for (const auto& set_data : sd->runeSets) {
		...
		if (SQL_ERROR == Sql_Query(mmysql_handle,
			"INSERT IGNORE INTO `runes` (`char_id`,`rune_id`,`set_id`,`selected`,`upgrade`,`failcount`) "
			"VALUES (%u, %u, %u, %u, %u, %u) "
...
void rune_load(map_session_data* sd) {
    ...
    if (Sql_Query(mmysql_handle,
        "SELECT `rune_id`, `set_id`, `selected`, `upgrade`, `failcount` "
        "FROM `runes` "
        "WHERE `char_id` = %d",
	...
	if(!isSelected){
		...
		// Shakto Fix: Calculate bookNumber immediately upon loading if a set is selected
		rune_count_bookactivated(sd, sd->runeactivated_data.tagID, sd->runeactivated_data.runesetid);
	}

}

int rune_bookactivate(map_session_data* sd, uint16 tagID, uint32 runebookid){
...
int rune_setactivate(map_session_data* sd, uint16 tagID, uint32 runesetid){
